class gogs::install (
  ...
  file { $repository_root:
    ...
    exec { 'download_gogs_from_github':
      ...
      unless => "/bin/bash -c 'DOWNLOAD_VERSION_URL=\"https://raw.githubusercontent.com/gogits/gogs/v\${PUPPET_GOGS_VERSION}/templates/.VERSION\" REMOTE_VERSION=\$(wget -O- -q \${DOWNLOAD_VERSION_URL}) LOCAL_VERSION=\$(<\${PUPPET_GOGS_INSTALLATION_DIRECTORY}/templates/.VERSION) && echo \"\${LOCAL_VERSION}\" && [[ \${LOCAL_VERSION} == \${REMOTE_VERSION} ]]'"
    }
